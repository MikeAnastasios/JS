...
function t5(event) {
    ...
    out5.innerHTML = i5.value.toUpperCase();
}
...
function t6(event) {
    ...
    document.querySelector('.i-6').value = i6;
    return false;
}
...
function t7() {
    ...
    out7.innerHTML = a7[randomInteger(0, a7.length-1)]; 
}

function randomInteger(min, max) {
    let i7 = min + Math.random() * (max + 1 - min);
    return Math.floor(i7);
}
...
let h = 75;
let w = 75;
let i10 = document.querySelector('.block-10');
function t10(event) {
    switch (event.keyCode) {
        case 40:
            h++;
            i10.style.height = h + 'px';
            break;
        case 38:
            h--;
            i10.style.height = h + 'px';
