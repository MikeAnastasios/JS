...
function t4() {
    a4_res = a4.filter(function(item, index) {
...
function t9() {
    ...
    a9.forEach(function(item) {
        return a9_res.push(item.toLowerCase());
    });
    a9 = a9_res;
    return a9_res;
}
...
function t14() {
    let a14_res = [];
    a14.map(function(item) {
        a14_res.push(item.toLowerCase())
    });
    let b14 = a14_sym.toLowerCase();
    return (a14_res.includes(b14));
}
